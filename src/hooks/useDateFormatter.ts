...
export function useDateFormatter() {
	const dateToString = (date: Date): string => {
		return date.toLocaleDateString(currentLocale, { year: 'numeric', month: 'short' });
	};

	const diffYearsMonths = (startDate: Date, endDate: Date): string => {
		...
		if (years > 0) parts.push(`${nf.format(years)} ${years === 1 ? "year" : "years"}`);
		if (months > 0) parts.push(`${nf.format(months)} ${months === 1 ? "month" : "months"}`);

		return parts.join(" ");
	};

	return { dateToString, diffYearsMonths };
}