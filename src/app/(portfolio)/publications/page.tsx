	...
	return (
		<main>
			...
			<div className={styles.cards_layout}>
				{publications.sort(p => p.publishDate.getMilliseconds()).map(c => (
					<div key={c.id} className={styles.card}>
						<div className={styles.caption}>
							<strong>{c.title}</strong>
							{' '}
							<DisplayDateRange endDate={c.publishDate} className={styles.issue_date} />
						</div>

						<div className={styles.info}>
							<span className={styles.publisher}>{c.publisher}</span>
							<span className={styles.description}>{c.description}</span>
							<span className={styles.authors}>Authors: {c.authors.map(authorToString).join(' | ')}</span>
							<span className={styles.keywords}>Keywords: {c.keywords.join(' | ')}</span>
