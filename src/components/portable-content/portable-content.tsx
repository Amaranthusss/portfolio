...
import type { ImageBlock } from '../../../prisma/seeds/helpers/portableText.types';
import type { CodeBlock } from '../../../prisma/seeds/helpers/portableText.types';

const CodeBlockLazy: React.LazyExoticComponent<
  ({ language, children }: CodeBlockProps) => React.ReactNode
> = lazy(() =>
...
const PrimaryMark = ({ children }: PropsWithChildren) => (
  <PrimaryText>{children}</PrimaryText>
);

const BoldMark = ({ children }: PropsWithChildren) => <b>{children}</b>;

const StrongMark = ({ children }: PropsWithChildren) => (
...
const QuoteBlock = ({ children }: PropsWithChildren) => (
...
export const PortableContent = memo(function PortableContent({
  content,
}: PortableContentProps): React.ReactElement | null {
