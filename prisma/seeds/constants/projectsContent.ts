import { content, image, div, list, paragraph, text, bold } from "../helpers/portableText.dsl";
...
projectContents.set(ProjectSlug.ProductionSystemUtilizingAGV, {
	...
	[Locale.en]: content(
		...
		paragraph(
			text("Their main purpose is to transport parts between designated points called stations. Depending on the working conditions of the transport structure consisting of AGV vehicles, various communication solutions are used between controllers responsible for logical and motion control.")
		),

		paragraph(
			text("The project included the development of a system of cooperating PLC controllers enabling automatic AGV calls from operator stations. Each station was equipped with a barcode reader allowing the operator to order specific parts.")
		),

		image("images/jammnik.jpg", "Automated Guided Vehicle (AGV)", 800, 450),

		paragraph(
			text("The station software reads barcodes using a scanner and analyzes them, displaying the result on the HMI panel. This allows the operator to verify whether the scanned command should be executed or cancelled.")
		),

		paragraph(
			text("Each scanned code is displayed for 10 seconds together with the recognized command name. During this time the operator may confirm the execution of the command, which results in sending a signal to the AGV controller.")
		),

		paragraph(
			text("Communication between PLC controllers is implemented using the ModBus TCP protocol operating over TCP/IP through PROFINET interfaces.")
		),
