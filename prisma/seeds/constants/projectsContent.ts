import { bulletList, image, paragraph, span } from "../helpers/portableText";
...
import { Mark } from "../helpers/portableText";

const projectContents = new Map<ProjectSlug, { [key in Locale]: InputJsonValue }>();

export function getProjectContent(slug: ProjectSlug, locale: Locale): InputJsonValue {
...
projectContents.set(ProjectSlug.ProductionSystemUtilizingAGV, {
	[Locale.pl]: [
		...
		paragraph(
			span("Podczas testów użyto pojazdu AGV „Jammnik”, opracowanego w ramach projektu demonstracyjnego i szkoleniowego.")
		),

		image("/images/jammnik.jpg", "Automatyczny Wózek Sterowany (AGV)", 800, 450),

		paragraph(
			span("Oprogramowanie stacji odczytuje kody kreskowe przy użyciu czytnika i analizuje je, wyświetlając wynik na panelu HMI. Pozwala to na rozpoznanie polecenia i pokazanie informacji na panelu HMI. Wizualizacja została zaprojektowana tak, aby operator mógł szybko odczytać kod produktu lub polecenia i zdecydować o jego wykonaniu lub anulowaniu. Każdy kod (ważny lub nieważny) jest wyświetlany przez 10 sekund wraz z rozpoznaną nazwą polecenia.")
		),

		paragraph(
			span("W tym czasie operator może potwierdzić wykonanie kodu przyciskiem na panelu HMI, co powoduje wysłanie sygnału zewnętrznego do sterownika AGV wraz z identyfikatorem wiadomości. Jeśli stacja wykryje nieznany, lecz potwierdzony kod, wyświetlany jest komunikat z prośbą o jego zdefiniowanie.")
		),

		paragraph(
			span("Komunikacja między sterownikiem PLC stacji a sterownikiem PLC AGV odbywa się przy użyciu protokołu ModBus TCP wykorzystującego komunikację TCP/IP przez interfejsy PROFINET sterowników.")
		),

		image("/images/jammnik-agv-hmi.jpg", "Panel Human-Machine Interface (HMI)", 800, 623),

		paragraph(span("Stacja nr 1:", [Mark.Strong])),
		...
		paragraph(span("Stacja nr 2:", [Mark.Strong])),
		...
		paragraph(span("Pojazd AGV:", [Mark.Strong])),
	...
	[Locale.en]: [
		...
		paragraph(
			span("Their main purpose is to transport parts between designated points called stations. Depending on the working conditions of the transport structure consisting of AGV vehicles, various communication solutions are used between controllers responsible for logical and motion control.")
		),

		paragraph(
			span("The project included the development of a system of cooperating PLC controllers enabling automatic AGV calls from operator stations. Each station was equipped with a barcode reader allowing the operator to order specific parts.")
		),

		image("images/jammnik.jpg", "Automated Guided Vehicle (AGV)", 800, 450),

		paragraph(
			span("The station software reads barcodes using a scanner and analyzes them, displaying the result on the HMI panel. This allows the operator to verify whether the scanned command should be executed or cancelled.")
		),

		paragraph(
			span("Each scanned code is displayed for 10 seconds together with the recognized command name. During this time the operator may confirm the execution of the command, which results in sending a signal to the AGV controller.")
		),

		paragraph(
			span("Communication between PLC controllers is implemented using the ModBus TCP protocol operating over TCP/IP through PROFINET interfaces.")
		),

		image("images/jammnik-agv-hmi.jpg", "Human-Machine Interface (HMI) Panel", 800, 623),

		paragraph(span("Station No. 1:", [Mark.Strong])),
		...
		paragraph(span("Station No. 2:", [Mark.Strong])),
		...
		paragraph(span("AGV Vehicle:", [Mark.Strong])),
